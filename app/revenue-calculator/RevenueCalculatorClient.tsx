...
function Slider({ label, value, min, max, step, onChange, format }: { label: string; value: number; min: number; max: number; step: number; onChange: (value: number) => void; format: (value: number) => string }) {
  ...
  return (
    <label className="rc-slider">
      <span className="rc-slider-head"><span>{label}</span><strong>{format(value)}</strong></span>
      <span className="rc-track"><span className="rc-fill" style={{ width: `${pct}%` }} /><span className="rc-dot" style={{ left: `calc(${pct}% - 9px)` }} /><input aria-label={label} type="range" min={min} max={max} step={step} value={value} onChange={(event) => onChange(Number(event.target.value))} /></span>
      <span className="rc-range"><span>{format(min)}</span><span>{format(max)}</span></span>
    </label>
  );
}
        ...
        <section className="rc-shell">
          ...
          <div className="rc-grid">
            <div className="rc-card">
              <div className="rc-card-head"><span>Your Numbers</span><small>Interactive estimate</small></div>
              ...
              <p className="rc-note">Use conservative numbers. This does not promise results; it shows where revenue may already be leaking.</p>
            </div>
            ...
            <div className="rc-results">
              ...
              <div className="rc-card">
                <div className="rc-card-head blue"><span>If Marketech closes the leak</span></div>
                ...
                <p className="rc-note blue-note">Real results depend on call volume, offer strength, speed-to-lead, sales process, and local competition.</p>
              </div>

              <a className="btn btn-primary rc-cta" href="/#contact"><span><b>Want this connected to your website?</b><small>Book a free conversion audit with Marketech Digital.</small></span><i>→</i></a>
            </div>
          </div>
