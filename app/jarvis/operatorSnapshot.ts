...
const SNAPSHOT_AT = "2026-09-23T23:40:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh exact-head review of #66 predecessor 949296f5 found one final P1: request-level fallback fences could not durably bind a different predecessor ref. Current exact #66 head b9576665 repairs this by role-binding durable Reviewer session fences to either target or predecessor, reconciling any stored role before success or rollback, and adding predecessor crash/restart regressions; fresh exact-head certification is queued. Bridge remains not installed. In parallel, MemPalace #36 exact d137e14a keeps the proven no-network/hidden-runner containment and replaces ensurepip with pinned uv 0.8.17 offline/hash-required installation inside a pip-less venv; fresh certification is queued. Issue #123 remains the durable capability ledger. Hermes #121 is pinned to v2026.9.21 / d337b736 and AwesomeLLMApps #122 to c6228784. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Finish all five required #66 gates on unchanged b9576665; if green, resolve the 949296f5 predecessor-fence P1 with exact-run evidence and request fresh independent review. In parallel, finish MemPalace #36 certification on d137e14a; if its sealed sandbox plus install/portfolio/web audits are green, resolve the current filesystem-isolation/audit P1/P2 threads and request fresh review. Continue Hermes/AwesomeLLMApps bounded extraction through #121/#122/#123. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 advanced to exact b9576665 after Codex found the predecessor-ref fallback-fence P1 on 949296f5; durable fences are now role/ref-bound and predecessor restart recovery is regression-covered, with fresh exact-head CI queued. MemPalace #36 advanced to exact d137e14a after containment itself passed but ensurepip failed; sealed installation now uses pinned uv 0.8.17 offline/hash-required into a pip-less venv. Hermes/AwesomeLLMApps remain durably pinned under #121/#122/#123. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 current exact b9576665 carries the predecessor-bound recovery-fence repair and regressions; fresh exact-head certification is queued after the 949296f5 review P1. MemPalace #36 current exact d137e14a carries the pinned-uv sealed install repair and is queued for certification. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed; stale-SHA review is never approval." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "predecessor_fence_repaired_exact_head_ci_running",
