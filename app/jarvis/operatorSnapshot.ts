...
const SNAPSHOT_AT = "2026-09-22T20:43:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Current exact #66 head e3c18d4f is fully engineering-green: Trusted Machine Bridge CI 35781443956, Action Guard 35781444141, Phase-0 35781443962, AI Reviewer CI 35781443961 and Observability 35781443978 all SUCCESS. The latest restart-recovery P1/P2 findings are repaired and their threads resolved; fresh exact-head Codex re-review 5783825562 is now processing. Bridge is not installed. #46 exact 0ddb5e8e remains CI-green and independently clean; Reviewer is not installed. #40 exact 6e9148fd remains engineering-green with Runtime OFF/quarantined. Revenue-first commercial unlock remains #72 eeee4078 -> #68 1a51a0e7 -> #73 939f601f; outbound is held.",
    next: "Consume fresh exact-head Codex re-review 5783825562 for #66 e3c18d4f. If clean and the SHA remains unchanged, surface the bounded Founder exact-head Bridge install decision. Keep #46 exact 0ddb5e8e frozen clean behind #66; install it only through the live Bridge, then use the live Reviewer to review #40 before any Runtime/autonomy decision. Outbound and spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 e3c18d4f is fully engineering-green after closing cleanup-proof timeline and malformed action-running envelope gaps; fresh exact-head Codex re-review 5783825562 has been picked up. #46 exact 0ddb5e8e remains independently clean. The online operator snapshot is refreshed at each material P0 transition while the signed live mirror remains inactive.",
    history: [
      "#66 exact e3c18d4f adds temporal cleanup-proof fencing and exact action-running journal-envelope validation before cleanup; all required exact-head certification gates are SUCCESS and fresh review 5783825562 is processing.",
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 e3c18d4f is fully engineering-green after the newest P1/P2 restart-recovery repairs; fresh exact-head Codex review 5783825562 is processing. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge exact-head re-review", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"PR #66 exact e3c18d4f is fully engineering-green: Bridge 35781443956, Action Guard 35781444141, Phase-0 35781443962, AI Reviewer CI 35781443961 and Observability 35781443978 all SUCCESS. The latest P1/P2 restart-recovery findings are repaired/resolved and fresh exact-head Codex re-review 5783825562 is processing. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 e3c18d4f is fully engineering-green with fresh exact-head Codex re-review 5783825562 processing; #46 0ddb5e8e is exact-head CI green and independently clean. Passing CI or stale-SHA review is never approval, and neither component is installed." },
