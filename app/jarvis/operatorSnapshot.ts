...
const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Current exact #66 head 54e7ff42 contains the durable request-journal Reviewer-session fence plus zombie/reused-leader revalidation. All five required exact-head gates are SUCCESS: Bridge 35929717911, Action Guard 35929718094, Phase-0 35929717972, AI Reviewer CI 35929718078 and Observability 35929718002. Both latest P1 threads are resolved and fresh Codex review 5804267709 is requested; Bridge remains not installed. In parallel, MemPalace #36 exact db90ba4 reroutes hosted-minute-blocked companion audits to the isolated marketech-lab-audit runner and adds per-workflow cancel-in-progress so future stale heads cannot pile up; the sealed no-network memory proof remains authoritative and production memory/MCP/model/provider authority remains OFF. PR #33 exact 670c03e refreshes the persistent capability ledger and Issue #123 remains canonical. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Consume fresh Codex review 5804267709 on unchanged #66 exact 54e7ff42; only a clean exact-head verdict may advance to the separate Founder Bridge-install decision. In parallel, drain obsolete pre-concurrency MemPalace runs, certify #36 exact db90ba4 on the isolated Lab, then fix only real current failures and request fresh review before a bounded Memory Router adapter. Keep Issue #123 / PR #33 as the single external-capability memory source; Hermes first extraction is the memory-provider host contract only, not the Hermes runtime. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 exact 54e7ff42 is fully engineering-green across all five required gates; both latest P1 threads are resolved and fresh Codex review 5804267709 is requested. MemPalace exact db90ba4 reroutes safe audits to the self-hosted Lab and adds stale-run cancellation for future heads; one rerouted Web Intelligence audit already passed. `config/capability-integration-ledger-v1.json` remains canonical on PR #33 exact 670c03e with Issue #123; duplicate PR #124 is closed. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 exact 54e7ff42 has all five required gates green, both latest P1 threads resolved, and fresh review 5804267709 requested. MemPalace #36 exact db90ba4 is separately queued on the same isolated Lab runner with stale-run cancellation now encoded for future heads. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact db90ba4 keeps external manifest-digest anchoring plus mount/PID/network/Landlock isolation, reroutes hosted-capacity-blocked audits to marketech-lab-audit, and adds per-workflow cancel-in-progress so future obsolete heads stop consuming the shared runner. A rerouted Web Intelligence audit has already passed. The sealed no-network workflow is authoritative; real corpus ingestion and MCP/model/provider/write authority remain OFF." },
