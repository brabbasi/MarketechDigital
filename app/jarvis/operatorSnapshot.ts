...
const SNAPSHOT_AT = "2026-09-22T20:37:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh Codex review of predecessor #66 bfcd3f73 found two restart-recovery findings: P1 cleanup-proof timeline validation and P2 malformed action-running journal envelope fencing. Both are repaired on current exact #66 head e3c18d4f; fresh Bridge, Action Guard, Phase-0, AI Reviewer CI and Observability certification runs are queued. Bridge is not installed. #46 exact 0ddb5e8e remains CI-green and independently clean; Reviewer is not installed. #40 exact 6e9148fd remains engineering-green with Runtime OFF/quarantined. Revenue-first commercial unlock remains #72 eeee4078 -> #68 1a51a0e7 -> #73 939f601f; outbound is held.",
    next: "Drain exact-head certification for #66 e3c18d4f. If all required gates are green, resolve the two predecessor review findings with exact-head evidence and request a fresh independent review of e3c18d4f. Keep clean #46 exact 0ddb5e8e frozen behind #66; install it only through the live Bridge, then use the live Reviewer to review #40 before any Runtime/autonomy decision. Outbound and spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 advanced to e3c18d4f after fresh review found cleanup-proof timeline and malformed action-running envelope gaps. Both production paths are repaired with focused restart regressions and exact-head certification is queued. #46 exact 0ddb5e8e remains independently clean. The online operator snapshot is refreshed at each material P0 transition while the signed live mirror remains inactive.",
    history: [
      "#66 exact e3c18d4f adds temporal cleanup-proof fencing and exact action-running journal-envelope validation before cleanup; fresh exact-head certification is queued.",
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 e3c18d4f is current after repairing the newest P1/P2 restart-recovery findings; exact-head certification is queued. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge exact-head certification", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"PR #66 exact e3c18d4f repairs cleanup-proof timeline validation and malformed action-running journal fencing found by fresh review of bfcd3f73. Fresh Bridge 35781443956, Action Guard 35781444141, Phase-0 35781443962, AI Reviewer CI 35781443961 and Observability 35781443978 are queued. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 e3c18d4f is current source-repaired / exact-head certification queued after fresh bfcd3f73 review findings; #46 0ddb5e8e is exact-head CI green and independently clean. Passing CI or stale-SHA review is never approval, and neither component is installed." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "exact_head_repaired_certification_queued",
