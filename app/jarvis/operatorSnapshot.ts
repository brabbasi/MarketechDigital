...
const SNAPSHOT_AT = "2026-09-23T22:30:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh review of #66 exact 88db56ea found two further recovery races. Both are repaired on current exact #66 head 8674d44c: double launch-journal/recovery-fence persistence failure now still attempts full known-session reaping, and the journaled leader start-time is revalidated on every cleanup scan. All five required #66 exact-head gates are now SUCCESS on 8674d44c: Bridge 35927201569, Action Guard 35927201575, Phase-0 35927201567, AI Reviewer CI 35927201578 and Observability 35927201604. The two latest P1 threads are resolved and fresh exact-head Codex review 5803983888 is active. Bridge remains not installed. In parallel, MemPalace #36 is now exact af044a42: external manifest digest anchoring and the Landlock/mount/PID/network sandbox remain in the sealed path, and two stale legacy compatibility checks were repaired so the old Wave-1 workflow validates that retired execution paths cannot be invoked. The sealed no-network workflow is the authoritative functional gate; production memory/MCP/model/provider authority remains OFF. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Consume fresh Codex review 5803983888 on unchanged #66 exact 8674d44c; only a clean exact-head verdict may advance to the separate Founder Bridge-install decision. In parallel, run MemPalace #36 exact af044a42 through the authoritative sealed no-network proof, reconcile the remaining portfolio/web audit reds as separate cross-lane checks, then resolve the fresh P1/P2 isolation findings and request exact-head review before any read-only adapter promotion. Hermes first bounded extraction remains read-only session search behind the cleared Memory Router contract. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 exact 8674d44c has all five required exact-head gates green and fresh Codex review 5803983888 active. MemPalace exact af044a42 has the hardened sandbox plus repaired legacy compatibility guard; sealed no-network certification is the authoritative MemPalace gate. The persistent-memory/capability lane is now additionally durably tracked by `config/capability-integration-ledger-v1.json` on PR #33 exact 83b9956a, with explicit lifecycle stage/owner/blocker/next-gate entries for MemPalace, Hermes, Awesome-LLM-Apps and the other selected sources. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge exact-head review", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 8674d44c repairs the two latest race findings. Bridge 35927201569, Action Guard 35927201575, Phase-0 35927201567, AI Reviewer CI 35927201578 and Observability 35927201604 are all SUCCESS. Both P1 threads are resolved and fresh Codex review 5803983888 is active. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. Review of #66 exact 88db56ea found two further P1 races now repaired on 8674d44c; all five required gates are green and fresh review 5803983888 is active. MemPalace #36 exact af044a42 is separately running the sealed Landlock/no-network proof after repairing stale legacy guard assertions. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact af044a42 carries external manifest-digest anchoring, mount/PID/network isolation, private /proc and Landlock filesystem allowlisting. Two stale legacy guard assertions were repaired so retired Wave-1 execution paths cannot be invoked. The sealed no-network workflow is the authoritative functional gate; real corpus ingestion and MCP/model/provider/write authority remain OFF." },
{ id:"t44", title:"Capability integration ledger", projectId:"jarvis", project:"JARVIS", agent:"Agent Resource Manager", state:"live", detail:"PR #33 exact 83b9956a adds config/capability-integration-ledger-v1.json as durable integration memory. It records lifecycle stage, owner lane, blocker and next safe gate for MemPalace, Hermes Agent, Awesome-LLM-Apps, event-driven autonomy, Browser Use, MoneyPrinterTurbo, YouTube automation, OpenVoice/MeloTTS and OpenHuman. Discovery scans cannot silently install or promote sources." },
