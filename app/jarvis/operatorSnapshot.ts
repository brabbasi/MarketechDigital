...
const SNAPSHOT_AT = "2026-09-22T21:42:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh Codex review of #66 head 42dde3ab found two new P1 crash-convergence defects: cleanup could miss an unrecognized descendant surviving inside the old Reviewer session, and rollback could delete the failed target before durably restoring the predecessor selector. Both are repaired on current exact #66 head 1c7d3227. Recovery now tracks the entire validated Reviewer session through TERM/KILL and restores the predecessor selector before failed-target cleanup. New adversarial regressions cover a SIGTERM-resistant descendant and a simulated crash between selector restoration and target deletion. Fresh exact-head CI is queued/running. Bridge is not installed. #46 exact 0ddb5e8e remains CI-green and independently clean; Reviewer is not installed. #40 exact 6e9148fd remains engineering-green with Runtime OFF/quarantined.",
    next: "Require all five exact-head #66 gates to pass on 1c7d3227. Then resolve the two 42dde3ab P1 findings with exact-head evidence and request a fresh Codex review bound only to unchanged 1c7d3227. Only a clean verdict may advance to the governed Bridge-install step. Keep #46 exact 0ddb5e8e frozen clean behind #66; Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 predecessor 42dde3ab failed fresh review on two P1 crash-convergence paths. Both are repaired on exact 1c7d3227 and fresh exact-head certification is running; all 42dde3ab review evidence is stale and no Bridge install has occurred. #46 exact 0ddb5e8e remains independently clean. The online operator snapshot is refreshed at material P0 transitions while the signed live mirror remains inactive.",
    history: [
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 1c7d3227 is the current repaired head after two new P1 crash-convergence findings on 42dde3ab; fresh exact-head certification is running and all predecessor review evidence is stale. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge crash-convergence repair", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Fresh review of #66 predecessor 42dde3ab found two P1s: incomplete full-session Reviewer reaping and unsafe failed-target deletion ordering. Current exact 1c7d3227 repairs both with adversarial regressions; fresh exact-head certification is running. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 current exact 1c7d3227 carries the newest P1 crash-convergence repairs and is in fresh exact-head certification; all 42dde3ab and earlier review evidence is stale. #46 0ddb5e8e is exact-head CI green and independently clean. Neither component is installed." },
