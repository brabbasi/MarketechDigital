...
const SNAPSHOT_AT = "2026-09-23T21:58:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh review of #66 head 6bba545a found two P1 recovery defects: an unproven Reviewer session was not durably fenced across retries, and a persisted session could signal a reused leader PID without rechecking start-time. Both are repaired on current exact #66 head 88db56ea with a durable Bridge-side recovery fence plus leader PID/start-time reuse rejection; fresh exact-head certification is queued/running. Bridge is not installed. In parallel, MemPalace PR #36 exact 767e0f73 remains sealed/no-network engineering-green and fresh independent review 5803557274 is active; the canonical Memory Router and disabled-by-default MemPalaceReadOnlyAdapter already exist. #46 exact 0ddb5e8e remains clean/not installed and #40 remains Runtime OFF.",
    next: "Certify exact #66 88db56ea through Bridge, Action Guard, Phase-0, AI Reviewer CI and Observability; if green, resolve the two 6bba545a P1 findings with exact-head evidence and request fresh Codex review. In parallel, consume MemPalace #36 fresh review; if clean, advance the existing read-only MemPalace adapter through synthetic project-scope/provenance/retention/failure evaluation before any real corpus ingestion. Keep #46 exact 0ddb5e8e frozen clean behind #66; Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 exact 88db56ea carries both latest recovery P1 repairs and a corrected fence-regression fixture; Phase-0 and Action Guard are already SUCCESS while the remaining exact-head gates run. MemPalace #36 is simultaneously in fresh independent review, and its read-only Router adapter already exists disabled-by-default. No Bridge, Reviewer, Runtime, memory corpus or external authority has been activated.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge durable-session repair", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Fresh review of #66 exact 6bba545a found two P1s: unproven sessions were not durably fenced across retries, and a persisted session leader PID could be reused before signaling. Current exact 88db56ea repairs both with a durable recovery fence and strict leader start-time/session reuse checks. Exact-head certification is queued/running. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 current exact 88db56ea carries the newest recovery-fence/PID-reuse repairs and is in exact-head certification; stale-SHA review is never approval and all earlier #66 review evidence is stale. MemPalace #36 exact 767e0f73 is separately in fresh Codex review 5803557274. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact 767e0f73 is sealed/no-network engineering-green and fresh independent review 5803557274 is active. Canonical Memory Router plus disabled-by-default MemPalaceReadOnlyAdapter already exist. Next clean gate: synthetic project-scope/provenance/retention/failure evaluation and read-only adapter promotion; real company/client corpus ingestion, MCP/model/provider authority and production write authority remain OFF." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "latest_p1s_repaired_exact_head_ci_running",
