...
const SNAPSHOT_AT = "2026-09-24T22:31:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. We have now reached and exercised the first real governed Founder-local bootstrap path: clean-reviewed #66 head 9b026ad8 launched locally but failed closed before installation because of the confirmed Windows-to-WSL numeric-exit/CR framing defect; no Runtime authority was touched. That framing defect and the subsequent diagnostic-retention defects are repaired. Fresh exact-head review of predecessor 009f8bbf then found two bootstrap-transaction P2s only: concurrent launchers were not serialized before journal inspection, and retained diagnostics were not fsynced/content-bound before the failure journal became durable. Both repairs remain in the lineage. The latest real Founder-local attempt then exposed a local GitHub CLI compatibility defect: this machine supports `gh api --paginate` but not newer `--slurp`. Current exact #66 head 9ed54bd1 removes that dependency, uses a Python fail-closed page flattener after `--paginate`, and adds regression coverage. Fresh Codex review of predecessor 1fdef0fd found one bootstrap-retry P2: the per-process gate directory suffix had collapsed to a literal trailing dollar. Current exact #66 head 9ed54bd1 restores the true process-unique suffix and adds a regression locking it. Fresh exact-head certification is running. Bridge remains NOT installed. In parallel, MemPalace #36 exact 79df2683 is on the sealed/install/portfolio/web certification lane; Issue #123 is the durable capability ledger; Hermes #121 is pinned to upstream 749220ef (v0.21.5, MIT) and AwesomeLLMApps #122 to c6228784 (Apache-2.0). #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Finish all five required exact-head gates on #66 9ed54bd1, resolve the predecessor P2 with exact-head evidence, and request a fresh Codex review bound only to unchanged 9ed54bd1. If clean, issue only the newly rebuilt governed bootstrap artifact. Do not rerun the `260b45cc` package or any older package. A successful local launch must prove the live Bridge healthy before clean #46 is installed through it; then the live Reviewer reviews #40 before any Runtime/Autonomy Phase A decision. In parallel, continue MemPalace #36 certification and Hermes/AwesomeLLMApps bounded extraction through #121/#122/#123. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 has progressed beyond paper readiness into a real governed local bootstrap attempt. That first launch failed closed safely, and its Windows-to-WSL framing bug has been repaired. Current exact 9ed54bd1 keeps the bootstrap transaction repairs and fixes the newly confirmed local `gh api --slurp` incompatibility without weakening pagination or exact-head review validation. The predecessor 1fdef0fd review found one retry-directory P2; current exact 9ed54bd1 repairs it and exact-head certification is running. MemPalace #36 remains on its independent parallel certification lane; Hermes/AwesomeLLMApps remain durably pinned under #121/#122/#123. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated yet.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge bootstrap repair", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 9b026ad8 was fully certified and independently clean, but governed bootstrap result 5805132014 failed closed with deployment_performed=null. The Windows-to-WSL trailing-CR exit bug is reproduced and repaired on exact 3e2654dd; fresh five-gate certification is running. Old bootstrap must not be rerun. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 current exact 9b026ad8 carries the predecessor-bound recovery-fence repair and regressions; fresh exact-head certification is queued after the 949296f5 review P1. MemPalace #36 current exact 79df2683 carries the dynamic-root masking and bounded-work-tmpfs repairs with fresh exact-head certification running. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed; stale-SHA review is never approval." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "retry_gate_suffix_p2_repaired_exact_head_ci_running",
