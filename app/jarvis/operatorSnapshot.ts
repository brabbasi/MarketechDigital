...
const SNAPSHOT_AT = "2026-09-22T21:46:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh Codex review of #66 head 42dde3ab found two new P1 crash-convergence defects: cleanup could miss an unrecognized descendant surviving inside the old Reviewer session, and rollback could delete the failed target before durably restoring the predecessor selector. Both are repaired on current exact #66 head 1c7d3227. Recovery now tracks the entire validated Reviewer session through TERM/KILL and restores the predecessor selector before failed-target cleanup. New adversarial regressions cover a SIGTERM-resistant descendant and a simulated crash between selector restoration and target deletion. All five required exact-head gates are green. Fresh Codex review 5784694177 is requested. Bridge is not installed. #46 exact 0ddb5e8e remains CI-green and independently clean; Reviewer is not installed. #40 exact 6e9148fd remains engineering-green with Runtime OFF/quarantined.",
    next: "Consume fresh Codex review 5784694177 bound only to exact #66 head 1c7d3227. Only a clean verdict on the unchanged SHA may advance to the governed Bridge-install step. Keep #46 exact 0ddb5e8e frozen clean behind #66; Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 exact 1c7d3227 is now 5/5 engineering-green after repairing both 42dde3ab crash-convergence P1s. Those threads are resolved with exact-head evidence and fresh Codex review 5784694177 is requested; no Bridge install has occurred. #46 exact 0ddb5e8e remains independently clean.",
    history: [
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 1c7d3227 is the current repaired head after two P1 crash-convergence findings on 42dde3ab; all five required exact-head gates are green and fresh Codex review 5784694177 is requested. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge exact-head re-review", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 1c7d3227 repairs both latest P1 crash-convergence findings. Bridge, Action Guard, Phase-0, AI Reviewer CI and Observability are all SUCCESS. Threads are resolved and fresh Codex review 5784694177 is requested. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 exact 1c7d3227 is 5/5 engineering-green with fresh Codex review 5784694177 requested; stale-SHA review is never approval, and all predecessor review evidence is stale. #46 0ddb5e8e is exact-head CI green and independently clean. Neither component is installed." },
