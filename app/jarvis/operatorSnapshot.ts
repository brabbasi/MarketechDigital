...
const SNAPSHOT_AT = "2026-09-23T22:52:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Current exact #66 head d2fc2061 carries the latest request-fence-before-health and rollback-request-context repairs plus regressions. Fresh exact-head certification is running: AI Reviewer CI 35931608024 is SUCCESS while Bridge, Action Guard, Phase-0 and Observability are queued on the shared isolated runner; the separate AI Workforce failure remains outside Bridge authority evidence. Bridge remains not installed. In parallel, MemPalace #36 exact db90ba4 has all four relevant engineering gates green, both prior P1/P2 threads resolved with exact-head evidence, and fresh Codex review 5804408191 is active. Issue #123 + PR #33 remain the durable capability ledger; Hermes and AwesomeLLMApps are pinned/selective integration lanes, not replacement control planes. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Let the remaining exact-head #66 certification jobs finish on d2fc2061. If all five required gates are green, resolve the two 54e7ff42 P1 threads with the new exact-run evidence and request fresh Codex review on d2fc2061; only a clean unchanged-SHA verdict may advance to Founder Bridge-install approval. In parallel, consume fresh MemPalace review 5804408191 on db90ba4; if clean, stage the synthetic-only Marketech Memory Router provider-interface adapter using the bounded Hermes memory-provider lifecycle contract. Keep Issue #123 / PR #33 as the canonical external-capability memory source. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 advanced to exact d2fc2061 with the two latest recovery repairs and regressions; AI Reviewer CI is green and the remaining required certification jobs are queued on the shared isolated runner. MemPalace #36 exact db90ba4 is engineering-green, its prior P1/P2 threads are resolved, and fresh Codex review 5804408191 is active. Hermes #121 and AwesomeLLMApps #122 now carry their own pinned durable intake checkpoints under Issue #123 / PR #33. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 54e7ff42 is the current exact-head candidate: durable request-journal session fencing and zombie/reused-leader checks are implemented; all five required gates are SUCCESS and fresh Codex review 5804267709 is requested. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge exact-head review", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 54e7ff42 carries durable request-journal session fencing and reused/zombie leader protections. Bridge 35929717911, Action Guard 35929718094, Phase-0 35929717972, AI Reviewer CI 35929718078 and Observability 35929718002 are all SUCCESS. Both latest P1 threads are resolved and fresh Codex review 5804267709 is requested. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 exact d2fc2061 carries the latest request-fence/rollback-context repairs; AI Reviewer CI is green and the remaining required certification jobs are queued. MemPalace #36 exact db90ba4 has all relevant engineering gates green and fresh review 5804408191 active. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact db90ba4 keeps external manifest-digest anchoring plus mount/PID/network/Landlock isolation. MemPalace Sealed No-Network, compatibility guard, Portfolio audit and Web Intelligence audit are all SUCCESS; the prior P1/P2 threads are resolved and fresh Codex review 5804408191 is active. Real corpus ingestion and MCP/model/provider/write authority remain OFF." },
