...
const SNAPSHOT_AT = "2026-09-22T22:08:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh Codex review of predecessor #66 head ab65ff73 found one remaining P1: if durable launch-session journaling itself failed after the Reviewer spawned a descendant that changed PGID, that descendant could survive without recovery evidence. Current exact #66 head 8f5558ea repairs that failure path by boundedly identity-tracking and reaping every process in the known Reviewer session even when journal persistence fails. Fresh exact-head certification is running; Action Guard is already green. Bridge is not installed. #46 exact 0ddb5e8e remains CI-green and independently clean; Reviewer is not installed. #40 exact 6e9148fd remains engineering-green with Runtime OFF/quarantined.",
    next: "Require all five exact-head #66 gates to pass on 8f5558ea. Then resolve the ab65ff73 launch-journaling P1 with exact-head evidence and request fresh Codex review on unchanged 8f5558ea. Only a clean verdict may advance to the governed Bridge-install step. Keep #46 exact 0ddb5e8e frozen clean behind #66; Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 predecessor ab65ff73 failed fresh review on one launch-journaling P1. Current exact 8f5558ea reaps the full known Reviewer session if persistence fails, including descendants that changed PGID; fresh exact-head certification is running and no Bridge install has occurred. #46 exact 0ddb5e8e remains independently clean.",
    history: [
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 8f5558ea is the current repaired head after the latest launch-journaling P1 on ab65ff73; full-session cleanup on journal-persistence failure is implemented, with fresh exact-head certification running. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge durable-session repair", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Fresh review of #66 predecessor ab65ff73 found one P1 in the launch-journal failure path. Current exact 8f5558ea reaps every identity-tracked process in the known Reviewer session even when durable launch-session persistence fails. Fresh exact-head certification is running. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 current exact 8f5558ea carries the newest P1 durable-session repairs and is in fresh exact-head certification; stale-SHA review is never approval, and all 1c7d3227 and earlier review evidence is stale. #46 0ddb5e8e is exact-head CI green and independently clean. Neither component is installed." },
