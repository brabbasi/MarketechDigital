...
const SNAPSHOT_AT = "2026-09-22T21:58:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh Codex review of #66 head 1c7d3227 found two deeper P1 crash-convergence defects: a descendant could change process groups while staying in the Reviewer session, and session identity was not durable across a Bridge crash after TERM. Both are repaired on current exact #66 head ab65ff73. Reviewer launch session/start identity is persisted before health checking/signaling; restart recovery consults that marker even when recognized commands are gone; full-session scans include descendants regardless of PGID and signal each identity-tracked member individually. Fresh exact-head certification is running; AI Reviewer CI is already green. Bridge is not installed. #46 exact 0ddb5e8e remains CI-green and independently clean; Reviewer is not installed. #40 exact 6e9148fd remains engineering-green with Runtime OFF/quarantined.",
    next: "Require all five exact-head #66 gates to pass on ab65ff73. Then resolve the two 1c7d3227 P1 findings with exact-head evidence and request fresh Codex review on unchanged ab65ff73. Only a clean verdict may advance to the governed Bridge-install step. Keep #46 exact 0ddb5e8e frozen clean behind #66; Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 predecessor 1c7d3227 failed fresh review on two deeper session-persistence P1s. Both are repaired on exact ab65ff73 with durable Reviewer launch-session identity and PGID-independent full-session recovery; fresh exact-head certification is running and no Bridge install has occurred. #46 exact 0ddb5e8e remains independently clean.",
    history: [
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 ab65ff73 is the current repaired head after two deeper P1 session-recovery findings on 1c7d3227; durable launch-session identity and PGID-independent full-session recovery are implemented, with fresh exact-head certification running. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge durable-session repair", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Fresh review of #66 predecessor 1c7d3227 found two P1s: descendants changing PGID could escape cleanup, and Reviewer session identity was not durable across a crash. Current exact ab65ff73 repairs both with durable launch-session evidence and PGID-independent session recovery. Fresh exact-head certification is running. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 current exact ab65ff73 carries the newest P1 durable-session repairs and is in fresh exact-head certification; stale-SHA review is never approval, and all 1c7d3227 and earlier review evidence is stale. #46 0ddb5e8e is exact-head CI green and independently clean. Neither component is installed." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "p1_durable_session_repaired_exact_head_ci_running",
