...
const SNAPSHOT_AT = "2026-09-23T22:46:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Current exact #66 head 54e7ff42 contains the durable request-journal Reviewer-session fence plus zombie/reused-leader revalidation; the stale test signatures are repaired. Bridge 35929717911, AI Reviewer CI 35929718078 and Action Guard 35929718094 are SUCCESS; Observability is running and Phase-0 is queued on the shared self-hosted Lab runner. Bridge remains not installed. In parallel, MemPalace #36 exact 07c9b3d has moved its hosted-minute-blocked Portfolio/Web Intelligence/legacy compatibility jobs onto the isolated marketech-lab-audit runner; its sealed no-network memory proof remains authoritative and production memory/MCP/model/provider authority remains OFF. PR #33 exact 670c03e refreshes the persistent capability ledger, while Issue #123 remains the canonical integration backlog. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Finish the remaining #66 54e7ff42 Observability + Phase-0 gates; if all five required exact-head gates are green, resolve the latest P1 threads with exact-run evidence and request a new exact-head Codex review. In parallel, let MemPalace #36 exact 07c9b3d consume the shared Lab runner, then fix only any real current failures and request fresh review before a bounded Memory Router adapter. Keep Issue #123 / PR #33 as the single external-capability memory source; Hermes and other selected repos advance one bounded capability at a time. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 exact 54e7ff42 is partially certified: Bridge, AI Reviewer CI and Action Guard are green; Observability is running and Phase-0 queued. MemPalace exact 07c9b3d reroutes its safe companion audits from exhausted hosted capacity to the isolated self-hosted Lab. `config/capability-integration-ledger-v1.json` is refreshed on PR #33 exact 670c03e; duplicate PR #124 was closed and Issue #123 remains canonical. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const agentOverrides: Record<string, Partial<JarvisAgent>> = {
  ...
  reviewer: {
    ...
    history: [
      ...
      "#66 54e7ff42 is the current exact-head candidate: durable request-journal session fencing and zombie/reused-leader checks are implemented; Bridge, AI Reviewer CI and Action Guard are green while Observability/Phase-0 finish. #46 0ddb5e8e remains exact-head CI green with clean fresh Codex review 5768921053.",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge exact-head certification", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 54e7ff42 carries durable request-journal session fencing and reused/zombie leader protections. Bridge 35929717911, AI Reviewer CI 35929718078 and Action Guard 35929718094 are SUCCESS; Observability is running and Phase-0 queued. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 exact 54e7ff42 is in fresh exact-head certification after durable request-journal session fencing; Bridge, AI Reviewer CI and Action Guard are green while two self-hosted gates finish. MemPalace #36 exact 07c9b3d is separately queued on the same isolated Lab runner. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed." },
  ...
  { id:"t29", title:"External capability integration registry", projectId:"jarvis", project:"JARVIS", agent:"Agent Resource Manager", state:"review", detail:"Canonical Marketech OS Issue #123 now persists Hermes, selected AwesomeLLMApps items, vetted GitHub repos, stale intake PRs and un-employed agents. Standing rule: this lane advances in parallel at safe bounded intervals; every candidate must move through qualification, license/security, sandbox, independent review, install, register, employ and health proof rather than remaining chat-only or PR-only." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact 07c9b3d keeps external manifest-digest anchoring plus mount/PID/network/Landlock isolation and reroutes Portfolio, Web Intelligence and legacy compatibility checks from exhausted ubuntu-latest capacity to the isolated marketech-lab-audit runner. The sealed no-network workflow is authoritative; real corpus ingestion and MCP/model/provider/write authority remain OFF." },
{ id:"t44", title:"Capability integration ledger", projectId:"jarvis", project:"JARVIS", agent:"Agent Resource Manager", state:"live", detail:"Issue #123 + PR #33 exact 670c03e are the single canonical capability memory. The ledger now refreshes MemPalace 07c9b3d and durably registers Hermes, Awesome-LLM-Apps, Codebase Memory, OpenBot, AgentTrail, Causentra, event-driven autonomy, Browser Use, MoneyPrinterTurbo, YouTube automation, OpenVoice/MeloTTS and OpenHuman with lifecycle/blocker/next-gate fields. Duplicate PR #124 is closed; discovery cannot silently install or promote sources." },
  { id:"t43", title:"Hermes selective integration lane", projectId:"jarvis", project:"JARVIS", agent:"Agent Resource Manager", state:"review", detail:"NousResearch/hermes-agent remains pinned at 18029116 in the capability registry as a selective source for persistent-memory/session-search/skills/subagents/cron/MCP/messaging/sandbox/model-portability patterns. Telemetry collection/send stays OFF unless separately approved; capability-by-capability Lab -> Reviewer -> adapter -> canary promotion only. Awesome-LLM-Apps remains a reference/skill-extraction source, not a daemon." },
];
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "exact_head_partial_green_ci_running",
