...
const SNAPSHOT_AT = "2026-09-23T23:23:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Current exact #66 head 949296f5 includes the request-context recovery fix plus aligned regressions; Observability is green and the remaining required exact-head gates are queued on the shared isolated runner. Bridge remains not installed. In parallel, MemPalace #36 exact 2503369b now carries the latest filesystem-isolation hardening plus exact-head audit checkout fixes; its certification is queued. Issue #123 remains the durable capability ledger. Hermes #121 is pinned to v2026.9.21 exact commit d337b736 with MIT/license/runtime authority mapping started; AwesomeLLMApps #122 is pinned to c6228784 under Apache-2.0 for selective extraction only. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Let #66 exact-head certification finish on 949296f5. If all five required gates are green, close the current Bridge regression state with exact-run evidence and request fresh Codex review on unchanged 949296f5; only a clean verdict may advance to Founder Bridge-install approval. In parallel, let MemPalace #36 certify exact 2503369b; if green, resolve the current P1/P2 review threads and re-request independent review. Continue Hermes/AwesomeLLMApps read-only audit/extraction work through #121/#122/#123 without granting runtime authority. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 advanced to exact 949296f5 with the request-context production fix and matching regressions; fresh exact-head certification is queued behind the shared runner with Observability already green. MemPalace #36 advanced to exact 2503369b with a minimal tmpfs/chroot mount sandbox and exact-head audit checkout assertions; certification is queued. Hermes #121 and AwesomeLLMApps #122 now have concrete upstream pins and license boundaries recorded under Issue #123. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. #66 exact 949296f5 carries the current request-context recovery fix and aligned regressions; Observability is green and remaining certification gates are queued. MemPalace #36 exact 2503369b carries the latest sandbox/audit repairs and is queued for certification. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed; stale-SHA review is never approval." },
