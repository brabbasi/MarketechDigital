...
const SNAPSHOT_AT = "2026-09-25T20:16:56Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 91,
    now: "P0 finish chain has materially advanced: Trusted Bridge #66 replacement 41017ae is installed and LIVE; Trusted Independent Reviewer #46 exact 80be359b is installed, healthy and singleton; Bubblewrap and Codex ChatGPT login are present on the Trusted Ubuntu host. PR #40 remains exact 6e9148fd, draft/open/mergeable with both continuation and activation-closure CIs green. Its latest semantic-review attempts progressed through host, identity, packet-builder and Codex preflight gates; the remaining step is a fresh successful semantic verdict using the now-refreshed Codex login. Runtime and #80 autonomy remain OFF until that verdict and the separate activation acceptance gate. Parallel work is no longer treated as merely tracked: MemPalace #36 is green/clean and now unblocked for its least-privilege Memory Router synthetic pilot; Hermes #121 and AwesomeLLMApps #122 remain pinned selective-integration lanes; workforce #93 is green but waits on #40 for true dispatch; the online Founder Portal itself is being repaired because predecessor a210c2a3 had a snapshot syntax error.",
    next: "Run a fresh live #40 semantic review through the installed Reviewer using the refreshed ChatGPT login. In parallel, advance MemPalace from sealed-Lab evidence into the bounded Memory Router adapter/synthetic pilot, keep Hermes/AwesomeLLMApps extraction moving under #121/#122/#123, reconcile workforce #93 behind the Runtime gate, and keep the Founder Portal snapshot/build green on every material transition. If #40 returns PASS, proceed immediately to the governed event-driven Runtime activation/acceptance transaction, then #80 Phase A canary.",
    blocked: 1,
    lastUpdate: "Bridge #66 is LIVE at 41017ae; Reviewer #46 is LIVE at 80be359b. PR #40 remains Runtime OFF but is now the sole critical serial gate before event-driven continuation. Parallel truth: MemPalace #36 exact 79df2683 has sealed-memory/install/portfolio/web-intelligence SUCCESS and clean independent review; Hermes #121 is pinned for bounded capability extraction; AwesomeLLMApps #122 has concrete offline candidates; workforce #93 is CI-green; Financial Independence OS #117 is CI-green but stale; dashboard predecessor a210c2a3 failed build on an unescaped quote in this snapshot and is being repaired now.",
    history: [
      "#66 replacement 41017ae passed all required gates, fresh review and Founder-local bootstrap, and is live with finite Reviewer-install authority only.",
      "#46 Reviewer 80be359b is current, healthy, singleton and limited to continuation_review_candidate + host_persistence_review_candidate.",
      "#40 exact 6e9148fd is draft/open with Event-Driven Continuation CI 35543685085 and Activation Closure CI 35543685091 SUCCESS; Runtime remains OFF pending a clean live semantic review and governed activation acceptance.",
      "MemPalace #36 exact 79df2683 is independently clean and bounded-test green; live Bridge + Reviewer prerequisites are now satisfied for the next Memory Router synthetic/read-only engineering stage.",
      "Hermes #121 and AwesomeLLMApps #122 remain selective capability sources under JARVIS, never replacement control planes.",
      "Founder Portal predecessor a210c2a3 failed build because operatorSnapshot.ts embedded an unescaped quote; this successor repairs the syntax and refreshes company truth instead of leaving the dashboard stale."
    ],
  },
  site: {
    state: "review",
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge governed install", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 4e030d19 is fully green and Codex review 5823835009 is clean. Governed bootstrap artifact 10838162352 from Bridge CI 36071283885 is frozen to this exact SHA and is the only allowed launcher. Next proof is Founder-local bootstrap success plus durable GitHub result; Bridge remains NOT installed until that proof exists." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical and stale-SHA review is never approval. #66 Bridge 41017ae is installed/live and #46 Reviewer 80be359b is installed/healthy. #40 exact 6e9148fd remains the active semantic-review gate with Runtime OFF; its latest retry failed closed because the pinned Codex/Node tool path is missing on the Trusted host. MemPalace #128 synthetic Memory Router pilot is now CI-green in parallel; no real corpus or authority is enabled." },
  ...
  { id:"t29", title:"External capability integration registry", projectId:"jarvis", project:"JARVIS", agent:"Agent Resource Manager", state:"review", detail:"Canonical Issues #120/#123 persist the external capability backlog and safe promotion cadence. Hermes #121 is pinned to upstream 749220ef (v0.21.5, MIT) for bounded-worker audit; AwesomeLLMApps #122 is pinned to c6228784 (Apache-2.0) for selective pattern extraction; MemPalace v3.8.0 remains pinned at 87e6f383 through #26/#36. Standing rule: advance this lane in parallel, but require qualification, license/security, sandbox, review, install, register, employ and health proof before ACTIVE." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact 79df2683 is the current sealed-memory candidate. Fresh review of predecessor d137e14a found two sandbox hardening issues: dynamic runner roots were not derived/masked and writable work tmpfs lacked an explicit cap. Both are repaired: actual runner top-level roots are derived and masked fail-closed, and writable work is a 512 MiB / 65,536-inode tmpfs. Exact-head sealed/install/portfolio/web gates are green; latest P1/P2 threads are resolved and fresh Codex review 5805079658 is requested. Promotion remains OFF. Real corpus ingestion and MCP/model/provider/write authority remain OFF." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "exact_head_clean_governed_install_ready",
