...
const SNAPSHOT_AT = "2026-09-23T22:17:00Z";

const projectOverrides: Record<string, Partial<JarvisProject>> = {
  jarvis: {
    state: "review",
    progress: 82,
    now: "P0 finish chain remains #66 -> #46 -> #40 -> #80. Fresh review of #66 exact 88db56ea found two further recovery races. Both are repaired on current exact #66 head 8674d44c: double launch-journal/recovery-fence persistence failure now still attempts full known-session reaping, and the journaled leader start-time is revalidated on every cleanup scan. Trusted Machine Bridge CI 35927201569 and Observability 35927201604 are SUCCESS on this head; Action Guard, Phase-0 and AI Reviewer CI are queued behind a stale superseded runner job. Bridge remains not installed. In parallel, MemPalace #36 advanced to exact efb76acd: external manifest digest anchoring is regression-green, the failed rootless bind-mount approach was replaced with user/mount/PID/network namespaces plus Landlock filesystem allowlisting, private /proc, zero capabilities and negative checkout/write probes, and the final device-node allowlist fix is queued for real sealed CI. #46 remains clean/not installed and #40 remains Runtime OFF.",
    next: "Finish exact #66 8674d44c certification through Action Guard, Phase-0 and AI Reviewer CI; if all green, reply to the two latest 88db56ea review findings with exact-head evidence and request fresh Codex review on 8674d44c. In parallel, run MemPalace #36 exact efb76acd through the sealed no-network Landlock proof; if clean, resolve the P1/P2 isolation findings and request fresh exact-head review before promoting the existing read-only MemPalace adapter. Hermes first bounded extraction remains read-only session search behind the cleared Memory Router contract. Runtime/autonomy/outbound/spend remain OFF.",
    blocked: 2,
    lastUpdate: "#66 exact 8674d44c has the real adversarial Bridge suite SUCCESS plus Observability SUCCESS; remaining exact-head gates are queued. MemPalace exact efb76acd has the corrected runner-compatible Landlock sandbox queued after the Bridge certification. The persistent-memory/capability lane remains durably tracked in Issues #26 and #123. No Bridge, Reviewer, Runtime, real memory corpus or external authority has been activated.",
    history: [
...
const operatorTasks: JarvisTask[] = [
  { id:"t1", title:"Trusted Bridge durable-session repair", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"#66 exact 8674d44c repairs the two latest race findings: double persistence failure still reaps the known session, and every cleanup scan revalidates the journaled leader start-time. Trusted Machine Bridge CI 35927201569 and Observability 35927201604 are SUCCESS; Action Guard, Phase-0 and AI Reviewer CI are queued. Bridge remains NOT installed." },
  ...
  { id:"t16", title:"Independent review queue", projectId:"jarvis", project:"JARVIS", agent:"AI Reviewer", state:"review", detail:"Issue #109 is canonical. Review of #66 exact 88db56ea found two further P1 races now repaired on 8674d44c; stale-SHA review is never approval. Fresh review waits for all exact-head gates to turn green. MemPalace #36 exact efb76acd is separately queued for sealed Landlock proof after its earlier P1/P2 isolation findings. #46 0ddb5e8e remains exact-head CI green and independently clean. Nothing is installed." },
  ...
  { id:"t42", title:"MemPalace persistent memory lane", projectId:"jarvis", project:"JARVIS", agent:"Memory Router Specialist", state:"review", detail:"PR #36 exact efb76acd carries the fresh isolation repair: external acquisition-manifest digest anchoring plus user/mount/PID/network namespaces, private /proc, Landlock filesystem allowlisting, zero capabilities, checkout/sibling-state negative read probes, sealed-input write-denial probes and bounded writable work only. Static regressions pass; real sealed self-hosted CI is queued. Canonical Memory Router plus disabled-by-default MemPalaceReadOnlyAdapter already exist; real corpus ingestion and MCP/model/provider/write authority remain OFF." },
...
export const operatorJarvisState: JarvisState = {
  ...
  tasks: operatorTasks,
  finishChain: {
    bridge: "bridge_ci_green_remaining_exact_head_gates_queued",
