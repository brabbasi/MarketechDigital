...
export const demoJarvisState: JarvisState = {
  ...
  agents: [
    ...
    { id:"engineering", name:"Engineering Agent", short:"EN", department:"Engineering", state:"running", x:57, y:82, load:78, task:"Live cockpit + remote Founder portal convergence", skills:["Next.js","Python","CI","systems"], workers:[{name:"Codebase Memory",state:"blocked-runtime",last:"Runtime recovery gated"},{name:"Graft Structure Specialist",state:"installed-reverify",last:"identity/scope reverify"},{name:"Codex Engineering Senior",state:"installed-reverify",last:"identity/scope reverify"}], history:["Founder cockpit v9 deployed through Trusted pull lane","Founder Portal rolling Vercel alias is deployed + browser-QA green; exact build SHA is exposed dynamically","Private Vercel Blob mirror driver is implemented behind OIDC/store gates and remains unconnected","Signed sanitized mirror publisher #104 is engineering-green and not activated"] },
  ...
  projects: [
    ...
    },
    {
      ...
      now:"Product head is frozen because GitHub-hosted private CI is out of reserve at 3000/3000 minutes; this is infrastructure-held, not a Rangrez product failure", next:"After independent review, use #101 repo-scoped self-hosted CI without consuming hosted minutes; otherwise the hosted reserve resets October 1. Keep product tests frozen until compute is available.",
    ...
    },
    {
      ...
      now:"Release head is frozen because GitHub-hosted private CI is out of reserve at 3000/3000 minutes; this is infrastructure-held, not a DeutschPath test verdict", next:"After independent review, use #101 repo-scoped self-hosted CI without consuming hosted minutes; otherwise the hosted reserve resets October 1. Only an executed test failure reopens product debugging.",
    ...
    },
    {
      ...
      now:"PR #5 remains canonical and frozen because GitHub-hosted private CI is out of reserve at 3000/3000 minutes; this is infrastructure-held, not a TradePilot product verdict.", next:"After independent review, use #101 repo-scoped self-hosted CI without consuming hosted minutes; otherwise the hosted reserve resets October 1. Preserve Safety Mode and inactive Supabase boundary.",
  ...
  tasks: [
    ...
    { id:"t13", title:"Private product CI runner fallback", projectId:"jarvis", project:"JARVIS", agent:"Agent Resource Manager", state:"review", detail:"GitHub-hosted private reserve is exhausted at 3000/3000 minutes. PR #101 is engineering-green and designed to bypass that hosted-minute dependency with isolated repo-scoped self-hosted runners; none is installed. Operational impact: affected private product CI is held, not the Company Runtime." },
