...
export const demoJarvisState: JarvisState = {
  ...
  agents: [
    ...
    { id:"revenue", name:"Revenue Agent", short:"RV", department:"Revenue", state:"running", x:16, y:63, load:49, task:"Research + pipeline movement", skills:["qualification","research","pipeline"], workers:[{name:"Revenue Assessment Trainee",state:"evaluation",last:"current"}], history:["69 qualified prospects in canonical queue","33 newest packets await independent review","Five directly traceable outreach threads rechecked with zero replies","Outbound remains governed and held"] },
  ...
  projects: [
    {
      ...
      now:"Cockpit v9 live; remote Founder portal 495e8a6d is deployed and QA-green with private Actions capacity truth surfaced; #93/#98/#101/#104 remain engineering-green", next:"Issue #109 is the shared review-capacity gate; keep #66 frozen as priority #1, and connect one private Marketech Blob store only after governed approval before any #104 publisher activation.",
      blocked:1, repo:"brabbasi/Marketech_Digital_OS", lastUpdate:"Remote Founder portal 495e8a6d is deployed and QA-green; issue #109 tracks the frozen independent-review queue; #104 publisher is hardened/green but no Blob store/write secret/ingest activation exists",
    ...
    },
    {
      ...
      now:"Founder Portal 495e8a6d is deployed on the rolling Vercel alias with Next 15.5.24, browser QA, Founder Truth strip, Actions-capacity truth and private Blob driver green", next:"Keep auth/consequence controls gated; clear #15/#16/#17 through issue #109 and connect one private Marketech Blob store only after governed approval before signed snapshot publication.",
      blocked:1, repo:"brabbasi/MarketechDigital", lastUpdate:"PR #13 exact 495e8a6d is deployed and QA-green; #15/#16/#17 are engineering-green and review-gated in issue #109; private Blob store remains unconnected",
  ...
  tasks: [
    ...
    { id:"t2", title:"Remote Founder portal delivery checkpoint", projectId:"site", project:"Website", agent:"Engineering Agent", state:"done", detail:"Exact bdb4c226 is deployed on the rolling Vercel alias; Next 15.5.24 build, desktop/mobile browser QA and fail-closed private Blob mirror driver are green." },
    ...
    { id:"t5", title:"Revenue research review batch", projectId:"jarvis", project:"Revenue", agent:"Revenue Agent", state:"queued", detail:"30 newest packets await independent review; 66 qualified total; five direct outreach threads rechecked with zero replies; no outbound." },
    ...
    { id:"t15", title:"Signed remote mirror publisher", projectId:"jarvis", project:"JARVIS", agent:"Engineering Agent", state:"review", detail:"PR #104 publisher is hardened and green; JARVIS private Blob driver is deployed fail-closed. Store connected=false, write secret configured=false and ingestion enabled=false." }
  ],
  revenue: {
    qualifiedProspects: 69,
    pendingIndependentReview: 33,
