...
export default function ContactPage() {
  return (
    <main className="contact-page">
      <section className="contact-shell" aria-labelledby="contact-title">
        ...
        <form className="contact-card" action="/api/lead" method="post">
          <div className="form-grid">
            ...
            <label>What do you need help with?<select name="service" defaultValue=""><option value="" disabled>Select one</option>{needs.map((item) => <option key={item}>{item}</option>)}</select></label>
          </div>
      ...
      <style jsx>{`
        .contact-page{min-height:100vh;background:radial-gradient(circle at 20% 0%,rgba(255,106,0,.14),transparent 34%),radial-gradient(circle at 80% 8%,rgba(89,175,255,.12),transparent 34%),#05070d;color:#fff;padding:120px 20px 80px}.contact-shell{width:min(980px,100%);margin:0 auto}.back-link{color:rgba(255,255,255,.72);text-decoration:none}.contact-label{display:inline-flex;margin-top:34px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);border-radius:999px;padding:9px 13px;color:rgba(255,255,255,.78);font-size:.82rem}h1{font-size:clamp(2.7rem,7vw,5.8rem);line-height:.9;letter-spacing:-.075em;margin:22px 0}p{max-width:760px;color:rgba(255,255,255,.72);line-height:1.7;font-size:1.1rem}.contact-card{margin-top:34px;border:1px solid rgba(255,255,255,.12);background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.035));border-radius:30px;padding:24px;box-shadow:0 24px 80px rgba(0,0,0,.28);display:grid;gap:18px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}label{display:grid;gap:8px;color:rgba(255,255,255,.82);font-weight:700}input,select,textarea{width:100%;box-sizing:border-box;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.22);color:#fff;border-radius:16px;padding:13px 14px;font:inherit}textarea{resize:vertical}button{border:0;border-radius:999px;padding:15px 20px;background:linear-gradient(135deg,#ff6a00,#ff8a2a);color:#120804;font-weight:900;cursor:pointer}small{color:rgba(255,255,255,.56);line-height:1.6}@media(max-width:760px){.form-grid{grid-template-columns:1fr}.contact-page{padding-top:92px}}
      `}</style>
    </main>
  );
}
