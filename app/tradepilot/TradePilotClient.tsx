...
const steps = [
  ["Lead comes in", "A customer calls, submits a form, or reaches out after hours."],
  ["TradePilot responds", "The system sends an instant response and starts the qualification flow."],
  ["Lead gets qualified", "The customer shares service type, location, urgency, and issue details."],
  ["Team gets alerted", "Your team receives the lead details and the opportunity is tracked."]
];
  ...
  return (
    <main className="tp-page">
      <section className="tp-hero" aria-labelledby="tradepilot-title">
        <div className="tp-pill">A Marketech Digital system built for trades businesses</div>
        <h1 id="tradepilot-title">Recover missed calls before they become your competitor&apos;s jobs.</h1>
        <p>TradePilot AI helps HVAC, plumbing, roofing, and electrical companies respond instantly to missed calls, after-hours inquiries, and website leads — then qualify, track, and follow up automatically.</p>
      ...
      <section className="tp-section tp-grid two" aria-labelledby="problem-title">
        <div><div className="tp-label">The problem</div><h2 id="problem-title">Fast-response businesses win the lead.</h2><p>Customers often contact more than one contractor. Missed calls, after-hours inquiries, and slow website responses can quietly turn warm demand into lost opportunity.</p></div>
        <div className="tp-card"><ul><li>Missed calls become competitor opportunities.</li><li>After-hours inquiries often wait until morning.</li><li>Website leads go cold when response is slow.</li><li>Most teams do not track what slipped through the cracks.</li></ul><p className="tp-muted">Even a 20–30% missed or delayed response rate can create thousands in lost monthly opportunity.</p></div>
      </section>
      ...
      <section id="workflow" className="tp-section" aria-labelledby="works-title"><div className="tp-label">How it works</div><h2 id="works-title">A simple missed-lead recovery flow.</h2><div className="tp-grid four">{steps.map(([title, copy], index) => <article className="tp-card" key={title}><span className="tp-step">0{index + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></section>

      <section className="tp-section tp-grid two" aria-labelledby="demo-title"><div><div className="tp-label">Demo workflow</div><h2 id="demo-title">After-hours missed call recovery.</h2><p>TradePilot reduces manual chasing by responding, qualifying, following up, and routing warm opportunities to your team.</p></div><div className="tp-phone"><div className="msg ai">Hi, this is [Company Name]. Sorry we missed your call. What can we help with today?<br />1 — Repair<br />2 — Quote/Install<br />3 — Emergency<br />4 — Maintenance</div><div className="msg user">3 — Emergency</div><div className="msg ai">Thanks. What&apos;s the service address or nearest intersection?</div><div className="msg user">Near Hazeldean and Eagleson. Furnace stopped working.</div><div className="owner-alert"><b>New recovered lead</b><span>Service: Emergency HVAC · Urgency: High · Recommended next step: Call now.</span></div></div></section>

      <section className="tp-section tp-card tp-wide" aria-labelledby="routing-title"><div className="tp-label">Warm lead routing</div><h2 id="routing-title">Spend less time chasing. Spend more time with qualified opportunities.</h2><p>TradePilot AI handles the first response, qualification, reminders, and follow-up so your team is not manually chasing every missed call, form submission, or after-hours inquiry.</p><div className="tp-grid three"><div><b>Responds before the lead goes cold</b></div><div><b>Follows up automatically when customers do not reply</b></div><div><b>Alerts your team when the lead is warm or urgent</b></div></div></section>

      <section className="tp-section tp-grid four" aria-label="TradePilot packages"><article className="tp-card package"><h3>14-Day Pilot</h3><strong>$500 setup</strong><p>Test the system for 14 days. If you continue, the setup is credited toward your first month.</p></article><article className="tp-card package"><h3>Starter</h3><strong>$799/month</strong><p>Missed-call text-back, website form response, basic qualification, owner alerts, follow-up sequence, and monthly report.</p></article><article className="tp-card package"><h3>Growth</h3><strong>$1,400/month</strong><p>Everything in Starter, after-hours workflow, booking link integration, lead scoring, weekly report, and optimization.</p></article><article className="tp-card package"><h3>Premium</h3><strong>$2,500+/month</strong><p>Lead-flow audit, CRM pipeline setup, call tracking, multi-service routing, dashboard, and monthly optimization meeting.</p></article></section>

      <section className="tp-section tp-final" aria-labelledby="pilot-title"><h2 id="pilot-title">Want to test TradePilot AI for your trades business?</h2><p>Book a free digital growth audit and ask for a TradePilot pilot/demo. We&apos;ll review your current lead flow and recommend the clearest next move.</p><div className="tp-actions"><Link href="/contact?service=tradepilot">Book a Pilot</Link><Link href="/contact?service=tradepilot-demo">Ask for a Demo</Link></div></section>

      <style jsx>{`.tp-page{min-height:100vh;background:radial-gradient(circle at 20% 0%,rgba(255,106,0,.14),transparent 34%),radial-gradient(circle at 80% 10%,rgba(89,175,255,.12),transparent 32%),#05070d;color:#fff;padding:110px 20px 70px}.tp-hero,.tp-section{width:min(1180px,100%);margin:0 auto}.tp-hero{min-height:70vh;display:flex;flex-direction:column;justify-content:center}.tp-pill,.tp-label{width:max-content;max-width:100%;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:rgba(255,255,255,.78);border-radius:999px;padding:9px 13px;font-size:.82rem}h1{max-width:980px;font-size:clamp(3rem,8vw,6.8rem);line-height:.9;letter-spacing:-.08em;margin:22px 0}h2{font-size:clamp(2rem,5vw,4rem);line-height:.95;letter-spacing:-.055em;margin:14px 0}p{color:rgba(255,255,255,.72);line-height:1.7}.tp-hero p{max-width:760px;font-size:1.15rem}.tp-actions{display:flex;flex-wrap:wrap;gap:12px;margin-top:18px}.tp-actions a{border-radius:999px;padding:14px 18px;color:#fff;text-decoration:none;font-weight:800;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.07)}.tp-actions a:first-child{background:linear-gradient(135deg,#ff6a00,#ff8a2a);color:#120804}.tp-section{padding:62px 0}.tp-grid{display:grid;gap:18px}.two{grid-template-columns:1fr 1fr;align-items:center}.three{grid-template-columns:repeat(3,1fr)}.four{grid-template-columns:repeat(4,1fr)}.tp-card,.tp-phone{border:1px solid rgba(255,255,255,.12);background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.035));border-radius:28px;padding:24px;box-shadow:0 24px 80px rgba(0,0,0,.28);backdrop-filter:blur(16px)}.tp-card ul{padding-left:20px;color:rgba(255,255,255,.72);line-height:1.8}.tp-muted{font-size:.88rem;color:rgba(255,255,255,.55)}.tp-calc-grid{display:grid;grid-template-columns:.9fr 1.1fr;gap:18px;margin-top:24px}.tp-slider{display:block;margin:0 0 20px}.tp-slider span{display:flex;justify-content:space-between;gap:12px;margin-bottom:10px;color:rgba(255,255,255,.8)}.tp-slider strong,.tp-result>strong,.package strong{color:#ff8a2a}.tp-slider input{width:100%;accent-color:#ff6a00}.tp-result>strong{display:block;font-size:clamp(2.6rem,8vw,5.5rem);letter-spacing:-.07em;margin:10px 0 20px}.tp-result-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.tp-result-grid div,.tp-wide .three div{border:1px solid rgba(255,255,255,.1);border-radius:18px;padding:14px;background:rgba(0,0,0,.18)}.tp-step{color:#ff8a2a;font-weight:900}.tp-phone{display:flex;flex-direction:column;gap:12px}.msg{max-width:88%;border-radius:18px;padding:13px 15px;color:rgba(255,255,255,.86);line-height:1.5}.msg.ai{background:rgba(89,175,255,.13);border:1px solid rgba(89,175,255,.22)}.msg.user{background:rgba(255,106,0,.18);border:1px solid rgba(255,106,0,.28);align-self:flex-end}.owner-alert{border:1px solid rgba(255,255,255,.16);border-radius:20px;padding:15px;background:rgba(255,255,255,.08);display:grid;gap:6px}.tp-wide .three{margin-top:18px}.package strong{display:block;font-size:1.35rem;margin:8px 0}.tp-final{text-align:center;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);border-radius:34px;padding:44px}.tp-final p{max-width:720px;margin:0 auto}.tp-final .tp-actions{justify-content:center}@media(max-width:900px){.two,.three,.four,.tp-calc-grid,.tp-result-grid{grid-template-columns:1fr}.tp-page{padding-top:90px}.tp-hero{min-height:auto;padding:70px 0 40px}}`}</style>
    </main>
  );
}
